...
export const ProcessListSidebar = (props: any) => {
  const {
    container,
    filters = {},
    group = true,
    ...
    onClose: any,
    onFiltersChange: any,
    onGroupChange: (event: ChangeEvent<HTMLInputElement>, checked: boolean) => void,
    open: boolean
  } = props;
  ...
  const handleQueryChange = useCallback((event: any) => {
    ...
    onFiltersChange?.({
      ...filters,
      query: (queryRef.current as any)?.value || ''
    });
  ...
  const handleCustomerToggle = useCallback((event: any) => {
    let customers;
    ...
    if (event.target.checked) {
      ...
      customers = (filters.customers || []).filter((customer: any) => customer !== event.target.value);
    }
